...
3. Create a `.env` file in the `Backend` directory and populate it:
   ```env
   PORT=5000
   MONGO_URI=mongodb+srv://<username>:<password>@<cluster-url>/careeriq?retryWrites=true&w=majority
...
2. Install dependencies:
   ...
   npm install
   ```
3. *(Optional)* Confirm API endpoint connections are targeting `http://localhost:5000/api` inside `Frontend/src/services/api.js`.
